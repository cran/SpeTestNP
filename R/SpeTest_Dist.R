...
###############################################################################
#### Generate a sample from the test statistic distribution under the null ####
...
SpeTest_Dist<-function(eq,type="icm",norma="no",boot="wild",nboot=50,para=FALSE,
  ...
  if (inherits(eq,"lm") || inherits(eq,"nls")){
    ...
    name<-switch(type,"icm"="Bierens (1982) integrated conditional moment","zheng"="Zheng (1996)",
                 "esca"="Escanciano (2006)","pala"="Lavergne and Patilea (2008)",
                 "sicm"="Lavergne and Patilea (2012) smooth integrated conditional moment")
    final<-as.matrix(Sboot)
    
    if (norma==T){
      
      colnames(final)<-paste("Sample from the",name,"normalized test statistic distribution under the null")
      
    } else if (norma==F){
      
      colnames(final)<-paste("Sample from the",name,"test statistic distribution under the null")
